...
for tof in all_tofs():
    ...
    if tof.tof_type == "ec_activity":  # RDE mesurements are O2-saturated
        ...
        nernst_overpotential = potential - E0 \
            - R * STANDARD_TEMPERATURE / (4 * FARADAY_CONSTANT) * np.log(activity)

    print(f"{sample_name}, {potential} V, rate = {rate} mol/s, "
    ...
    ax_eta.plot(
        nominal_overpotential * 1e3, rate, color=color, marker=marker, fillstyle="none", alpha=0.2,
    )
    ax_eta_norm_r.plot(
        nominal_overpotential * 1e3, f, color=color, marker=marker, fillstyle="none", alpha=0.2,
    )
